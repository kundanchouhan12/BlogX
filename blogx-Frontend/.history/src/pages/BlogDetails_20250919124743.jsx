...
import { FaTrash, FaPaperPlane, FaEdit } from "react-icons/fa";
...
export default function BlogDetails() {
  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 p-6 bg-white rounded-xl shadow-md">
      ...
      {/* ✅ Update/Delete Buttons -> only if post belongs to logged in user */}
      {user && user.id === post.user?.id && (
        <div className="flex gap-4 mb-4">
          <button
            onClick={() => navigate(`/edit/${id}`)}
            className="bg-yellow-400 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-yellow-500"
          >
            <FaEdit /> Edit
          </button>
          <button
