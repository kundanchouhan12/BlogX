...
export default function Home() {
  ...
  const recPerPage = 3;
  const recTotalPages = Math.ceil(recommendedPosts.length / recPerPage);
  const [recPage, setRecPage] = useState(0);

  // Slice posts for current recommended page
  const displayedRecommended = recommendedPosts.slice(
    recPage * recPerPage,
    recPage * recPerPage + recPerPage
  );

  // Handlers
  const handleRecNext = () => {
    setRecPage((prev) => (prev + 1) % recTotalPages);
  };

  const handleRecPrev = () => {
    setRecPage((prev) => (prev - 1 < 0 ? recTotalPages - 1 : prev - 1));
  };
    ...
    <div className="flex flex-col min-h-screen bg-gray-50 font-sans">
      <main className="flex-grow pb-12">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full mt-12">
          ...
          {/* Recommended Section */}
          <div className="mt-16">
            <h2 className="text-3xl font-bold text-indigo-900 mb-4 tracking-tight">
              Recommended For You
            </h2>
            <div className="relative">
              ...
              <button
                ...
                className="absolute left-0 top-1/2 transform -translate-y-1/2 -translate-x-full z-10 p-3 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {displayedRecommended.map((post) => (
                  <div
                    key={post.id}
                    className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 border border-gray-100 flex flex-col"
                  >
                    <img
                    ...
                    <div className="p-5 flex flex-col justify-between flex-grow">
                      <div>
                        <h3 className="text-lg font-semibold text-indigo-800 mb-2 line-clamp-2">
                          {post.title}
                        </h3>
              ...
              <button
                ...
                className="absolute right-0 top-1/2 transform -translate-y-1/2 translate-x-full z-10 p-3 bg-indigo-600 text-white rounded-full hover:bg-indigo-700 transition"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mt-12">
            {displayedPosts.map((post) => (
              <div
                key={post.id}
                className="bg-white rounded-2xl shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col"
              >
                <img
                  src={post.imageUrl || "https://via.placeholder.com/400x200"}
                  alt={post.title}
