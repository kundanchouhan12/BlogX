...
const recPerPage = 3; // ek page me kitne posts show karne hain

// Calculated posts slice
const displayedRecommended = recommendedPosts.slice(recIndex, recIndex + recPerPage);

// Handlers
const handleRecNext = () => {
  setRecIndex((prev) =>
    prev + recPerPage >= recommendedPosts.length ? 0 : prev + recPerPage
  );
};
...
const handleRecPrev = () => {
  setRecIndex((prev) =>
    prev - recPerPage < 0
      ? Math.max(recommendedPosts.length - recPerPage, 0)
      : prev - recPerPage
  );
};
  ...
  <div className="flex items-center gap-4">
    ...
    <div className="flex gap-6 overflow-hidden w-full">
      {displayedRecommended.map((post) => (
        <div
          key={post.id}
          className="min-w-[280px] max-w-sm bg-white rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 flex-shrink-0 border border-gray-100"
        >
          <img
                ...
                <div className="p-4 flex flex-col justify-between flex-grow">
                  ...
                  <div className="flex justify-between items-center mt-4">
                    <div className="flex items-center text-gray-500 text-sm">
                      ...
                      <span>
                        By {post.username || user?.name || "Anonymous"}
                      </span>
                    </div>
                    <Link
                      to={`/posts/${post.id}`}
