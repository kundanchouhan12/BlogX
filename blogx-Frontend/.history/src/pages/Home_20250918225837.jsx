...
if (search.trim() !== "") {
  ...
  const authorMatches = [];
  const titleContentMatches = [];
  const unmatched = [];

  filteredPosts.forEach((p) => {
    const titleMatch = p.title.toLowerCase().includes(term);
    const contentMatch = p.content.toLowerCase().includes(term);
    const authorMatch = p.username.toLowerCase() === term; // exact author match

    if (authorMatch) {
      authorMatches.push(p);
    } else if (titleMatch || contentMatch) {
      titleContentMatches.push(p);
    } else {
      unmatched.push(p);
    }
  });

  filteredPosts = [...authorMatches, ...titleContentMatches, ...unmatched];
}
