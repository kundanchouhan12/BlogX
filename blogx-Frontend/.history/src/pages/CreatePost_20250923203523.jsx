...
//         <h2 className="text-4xl font-bold text-center text-indigo-700">✍️ Create a New Post</h2>
...
//                 <button type="button" onClick={handleRemoveImage} className="absolute z-10 px-3 py-2 bg-red-600 text-white rounded-full">Remove</button>
//               </>
//             ) : (
...
export default function CreatePost() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const formData = new FormData();
      formData.append("title", title.trim());
      formData.append("content", content); // Quill content is HTML
      if (category) formData.append("category", category.trim());
      if (image) formData.append("image", image);
  ...
  // Quill toolbar options (can be customized)
